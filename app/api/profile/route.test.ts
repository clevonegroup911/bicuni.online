...
const mocks = vi.hoisted(() => ({ auth: vi.fn(), university: vi.fn(), department: vi.fn(), transaction: vi.fn(), profileUpsert: vi.fn() }));
...
describe("PATCH /api/profile", () => {
  beforeEach(() => {
    ...
    mocks.transaction.mockImplementation(async (callback) => callback({
      user: { update: vi.fn(), findUniqueOrThrow: vi.fn().mockResolvedValue({ id: "user-1" }) },
