...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const document = await db.document.findUnique({ where: { id }, select: { authorId: true, status: true } });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const id = (await params).id; const document = await db.document.findUnique({ where: { id }, select: { authorId: true, status: true } });
  if (!document || !canReadDocument(session.user, document)) return NextResponse.json({ error: "Document introuvable." }, { status: 404 });
